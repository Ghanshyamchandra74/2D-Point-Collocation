...
tic;clear;clc;KAF = 3;[e] = SDPT(KAF);pretty(e);n=3;R=0;syms x y;
%***********************************************

Lx=60;Ly=(1);Lxx=Lx;Lyy=Ly;a=(100);a=a/2;

w = 50;%10 25 50
KK = 0.25;%;*(((Lxx-a)/Lyy)^2);% 1 0.5 0.25
%*********************************************

hs =20;%10.45+10*(x*w*((0.01)))^(0.5)-(x*w*((0.01)));

K1=4*KAF; %K1 = Sum of No of Elements
f1=K1;

Bis = (hs/((KK)^(2)*200))*(Lyy);
Bic = (10);%(0.1)*((Lxx-a)/Lyy);
...
if Lx>=Ly 
    L = Lx;point=2;
    Lx = linspace(a,Lxx,point+2);Lx=Lx';Ly = linspace(0,Lyy,point+1);Ly=Ly';
    for i=1:b
    if mod(i,2)==1
    for j=1:point-1
    ...
    for j = 1:point
        ...
        for j=1:point-1
    H(j+point-1,1) = subs(G(i,1),{x y},{Lxx Ly(j+1,1)});
    end
    for j = 1:point
...
if mod(f2,4) == 1
f2x=linspace(a,Lxx,point+3);f2x=f2x';f2y=linspace(0,Lyy,point+3);f2y=f2y';
    for i=1:f2/4
    J(i,1) = subs(R,{x y},{a f2y(i+3,1)});
    end
    
   i11 = length(J);
    
    for i=1:f2/4
    J(i+i11,1) = subs(R,{x y},{Lxx f2y(i+3,1)});
    end
      i12 = length(J);
    
    for i=1:f2/4
    J(i+i12,1) = subs(R,{x y},{f2x(i+3,1) 0});
    end
      i13 = length(J);
    for i=1:f2/4
    J(i13+i,1) = subs(R,{x y},{f2x(i+3,1) Lyy});
    end
else
    f2x=linspace(a,Lxx,f2+f2);f2x=f2x';f2y=linspace(0,Lyy,f2+f2);f2y=f2y';
    for i=1:f2
...
syms x y;F = P(1,1)+P(2,1)*x+P(3,1)*y+P(4,1)*(x)*(y);j=4;
for k=2:KAF
    j=j+1;
F = F +P(j,1)*(x^k)*y+P(j+1,1)*x*(y^k)+P(j+2,1)*(x^k)+P(j+3,1)*(y^k);j=j+3;
end
...
if Lxx>=Lyy
figure(2);[C3,h1] = contourf(xx,yy,zz,sd);clabel(C3,h1); hold on;
else
figure(2);[C3,h1] = contourf(yy,xx,zz,sd);clabel(C3,h1); hold on;    
end

figure(2);[C2,h2] = contourf(xx,yy1,zz,sd); hold on;colorbar;colormap jet;
title('Temperature Distribution (Normalised)');
xlabel('Radius(Normalised)');
ylabel('Thickness(Normalised)');
sd2=50;
sd3=10;
